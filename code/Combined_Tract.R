...
saveRDS(Tract_ACS,"./data/Tract_ACS.rds")
...
Tract_ACS <- readRDS("./data/Tract_ACS.rds")

vars_new <- read_excel("./Variables_to_Include_1.1.xlsx") # Read variable list (manually edited - BG_Variables.csv)
...
ag_vars_out <- Tract_ACS %>% #Create a table of GEOID
  select(GEOID)
...
prt %<>%
bind_cols(select(Tract_ACS,"GEOID"))



# Save the measures and description file
saveRDS(prt,"./data/TRACT_1.3_Pct.rds")

saveRDS(vars_new,"./data/Variables_to_Include_1.3.rds")
...
prt <- readRDS("./data/TRACT_1.3_Pct.rds")
vars_new <- readRDS("./data/Variables_to_Include_1.3.rds")
v_used <- vars_new %>% filter(Keep_Variables == 1) %>% select(V_name) %>% pull() # select proposed used variables
...
saveRDS(results,"./data/final_k250_Tract.rds")



clusters <- as_tibble(h2o.predict(results,cluster_variables_h20))

usa.tract.cl <- tibble(GEOID= prt$GEOID,cluster=clusters$predict)
...
#Plot Silhouette
ggplot(data=sil, aes(x=log(k), y=sil, label=k)) +geom_line() + geom_vline(xintercept=log(c(12,22,58)), lty=3, lwd=.5)

#Create cut points in the tree hierarchy and lookup 
ward.cuts <- data.frame(cluster=1:250, cutree(wards.ctr,k=c(12,22,58)))
...
write_csv(usa.tract.cl,"Clusters_Tracts.csv")
...
usa.tract.cl <- read_csv("Clusters_Tracts.csv")
vars_new <- readRDS("./data/Variables_to_Include_1.5.rds")
...
index_variables <- Tract_ACS %>%
select("GEOID","B01001_001",all_of(index_used))%>%
  left_join(usa.tract.cl)
 

X12 <- index_variables %>%
  select(-c(GEOID,cluster,X22,X58)) %>%
  group_by(X12) %>% 
...
all_pct <- X12 %>%
  ...
  Tract_SF <- map_df(state_codes, function(state_code) {
  ...
  #saveRDS(Tract_SF,"./data/Tract_SF.rds")
  ...
  Tract_SF <- readRDS("./data/Tract_SF.rds")
  ...
  st_write(obj = Tract_SF, dsn = "Tract_SF_1.3.gpkg")
