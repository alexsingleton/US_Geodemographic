...
usa.trt.cc <- usa.trt.bx[complete.cases(usa.trt), ]

#INCOMPELTE CASES (and count missing / problematic GEOID)
usa.trt.ic <- usa.trt.bx[!complete.cases(usa.trt), ]
...
saveRDS(results,"./data/results_10k_250_BX.rds")
...
usa.trt.cl <- tibble(GEOID= usa.trt[complete.cases(usa.trt), ]$GEOID,cluster=result$cluster)
...
#Plot Silhouette
ggplot(data=sil, aes(x=log(k), y=sil, label=k)) +geom_line() + geom_vline(xintercept=log(c(8,15,69)), lty=3, lwd=.5)

#Create cut points in the tree hierarchy 
ward.cuts <- data.frame(cluster=1:250, cutree(wards.ctr,k=c(8,15,69)))
...
write_csv(usa.trt.cl,"Clusters_BX.csv")
